...
include( dirname( __FILE__ ) . DS . 'models' . DS . 'event-posts.php' );
include( dirname( __FILE__ ) . DS . 'models' . DS . 'events.php' );
include( dirname( __FILE__ ) . DS . 'models' . DS . 'groups.php' );
include( dirname( __FILE__ ) . DS . 'models' . DS . 'group-taxonomy.php' );
include( dirname( __FILE__ ) . DS . 'models' . DS . 'options.php' );
include( dirname( __FILE__ ) . DS . 'models' . DS . 'api.php' );
include( dirname( __FILE__ ) . DS . 'controller.php' );
include( dirname( __FILE__ ) . DS . 'controllers' . DS . 'calendar_widget.php' );
include( dirname( __FILE__ ) . DS . 'controllers' . DS . 'events_widget.php' );
include( dirname( __FILE__ ) . DS . 'controllers' . DS . 'events_controller.php' );
...
class WP_Meetup {
	...
	function __construct() {
		...
		$this->dir = WP_PLUGIN_DIR . DS . 'wp-meetup' . DS;
		$this->plugin_url = plugins_url( DS, __FILE__ );
	...
	function admin_init() {
		...
		wp_register_style( 'farbtastic', plugins_url( DS . 'js' . DS . 'farbtastic' . DS . 'farbtastic.css', __FILE__ ) );
		wp_enqueue_style( 'farbtastic' );
		wp_register_script( 'farbtastic', plugins_url( DS . 'js' . DS . 'farbtastic' . DS . 'farbtastic.js', __FILE__ ), array( 'jquery' ) );
		wp_register_script( 'options-page', plugins_url( DS . 'js' . DS . 'options-page.js', __FILE__ ), array( 'jquery', 'farbtastic' ) );
	}
	...
	function cron_update() {
		...
		if ( WP_DEBUG ) {
			file_put_contents( dirname( __FILE__ ) . DS . 'data.txt', date( 'r' ) . ' ' . ( $status ? 'success' : 'failure' ) . "\n", FILE_APPEND );
		}
	}
	...
	function render( $filename, $vars = array() ) {
		if ( is_file( $this->dir . 'views' . DS . $filename ) ) {
			ob_start();
			extract( $vars );
			include $this->dir . 'views' . DS . $filename;
			return ob_get_clean();
		}
		return false;
	}
