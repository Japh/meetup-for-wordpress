...
class WP_Meetup_Events_Controller extends WP_Meetup_Controller {
	...
	function handle_post_data() {
			...
			if ( $parsed_name != '' ) {
				...
				if ( ! in_array( $parsed_name, $this->groups->get_url_names() ) ) {
					...
					if ( $group_data = $this->api->get_group( $parsed_name ) ) {
						...
						$this->feedback['error'][] = 'The Group URL you entered is not valid.';
					}

				} else {
					$this->feedback['error'][] = 'The group URL you entered refers to a group you have already added.';
				}
			}
		}
		...
		if ( array_key_exists( 'show_plug_probability', $_POST ) && $_POST['show_plug_probability'] != $this->options->get( 'show_plug_probability' ) ) {
			$this->options->set( 'show_plug_probability', $_POST['show_plug_probability'] );
			$this->feedback['message'][] = 'Successfully updated the probability of a link to Nuanced Media appearing on your event posts';
		}
